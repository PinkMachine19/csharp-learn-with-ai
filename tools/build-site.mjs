...
function renderSingleStructuredLab(lab, pasteReadyInstructions = false) { const steps = lab.steps.map((step, index) => renderLabStep(step, index, lab.stepCheckboxes, lab.steps.length, pasteReadyInstructions)).join(""); const instructions = lab.collapseInstructions ? `<details class="lab-instructions-reveal"><summary><strong>Instructions</strong><span class="quiz-expand-hint">Open instructions</span></summary>${steps}</details>` : `<h4>Instructions</h4>${steps}`; return `<article class="card lab-unit"><h3>Lab 1 of 1 — ${escapeHtml(lab.title || "Complete the Session Lab")}</h3><h4>Objective</h4><p class="lab-objective">${escapeHtml(lab.objective)}</p>${lab.practice ? `<h4>You’ll Practice</h4><ul>${lab.practice.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}${lab.task ? `<h4>Task</h4><p>${escapeHtml(lab.task)}</p>` : ""}${instructions}${lab.solution ? renderLabReveal({ title: "Solution", text: "Reveal only after completing the lab or when you are genuinely blocked.", code: lab.solution }) : ""}</article>`; }
