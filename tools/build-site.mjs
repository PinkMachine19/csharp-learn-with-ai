...
function sessionPage(session, content, track = sessions) {
  ...
  const optionalBanner = session.isSideLab ? `<div class="side-lab-notice"><strong>Optional Session ${String(session.attachedToSession).padStart(2,"0")} sidebar:</strong> ${escapeHtml(session.sidebarDescription)} It is safe to skip and does not block Session ${String(session.attachedToSession + 1).padStart(2,"0")}.</div>` : session.isSupplemental ? `<div class="refresher-notice"><strong>Optional reference:</strong> This session is not a prerequisite for the TimeClock application path. Use it when you need a modern C# reminder or interview review.</div>` : "";
...
function renderLabBody(lab) { return lab.labs ? lab.labs.map((item, index) => renderStructuredLab(item, index, lab.labs.length, lab.collapseInstructions)).join("") : lab.steps.map(renderLabStep).join(""); }
