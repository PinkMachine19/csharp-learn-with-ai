...
function renderStructuredLab(lab, index, total) { return `<article class="card lab-unit"><h3>Lab ${index + 1} of ${total} — ${escapeHtml(lab.title)}</h3><p class="lab-objective"><strong>Objective:</strong> ${escapeHtml(lab.objective)}</p>${lab.comment ? `<p><strong>Paste this comment into the exercise file before starting:</strong></p><pre><code>${escapeHtml(lab.comment)}</code></pre>` : ""}<h4>Instructions</h4><ol class="lab-instructions">${lab.instructions.map((instruction) => { const detail = typeof instruction === "string" ? { text: instruction } : instruction; return `<li><p>${escapeHtml(detail.text)}</p>${detail.code ? `<pre><code>${escapeHtml(detail.code)}</code></pre>` : ""}</li>`; }).join("")}</ol></article>`; }
