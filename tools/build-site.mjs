...
function labPage(session, content) { const label = session.isSupplemental ? `Refresher ${session.number}` : `Session ${String(session.number).padStart(2,"0")}`; return page({title:`${label} Lab — ${session.title}`,active:"labs",description:content.lab.objective,body:`<main id="main-content"><div class="container">
  ...
  ${content.lab.steps.map(renderLabStep).join("")}
...
function renderLabReveal(reveal) { return `<div class="reveal-card"><button type="button" aria-expanded="false"><span>${escapeHtml(reveal.title)}</span><small>Reveal</small></button><div hidden>${reveal.text ? `<p>${escapeHtml(reveal.text)}</p>` : ""}${reveal.items ? `<ul>${reveal.items.map((item)=>`<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}${reveal.code ? `<pre><code>${escapeHtml(reveal.code)}</code></pre>` : ""}</div></div>`; }
