...
function renderLabBody(lab) { const keyPoint = lab.keyPoints ? `<aside class="card lab-key-point"><h3>${escapeHtml(lab.keyPointTitle || "Key point")}</h3><ul>${lab.keyPoints.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>${lab.keyPointCode ? `<pre><code>${escapeHtml(lab.keyPointCode)}</code></pre>` : ""}</aside>` : ""; const body = lab.labs ? lab.labs.map((item, index) => renderStructuredLab(item, index, lab.labs.length, lab.collapseInstructions)).join("") : lab.steps.map((step, index) => renderLabStep(step, index, lab.stepCheckboxes)).join(""); const attributes = [lab.clickHighlightInstructions ? "data-click-highlight-instructions" : "", lab.stepCheckboxes ? "data-step-checkboxes" : ""].filter(Boolean).join(" "); const steps = attributes ? `<div ${attributes}>${body}</div>` : body; return `${keyPoint}${steps}`; }
