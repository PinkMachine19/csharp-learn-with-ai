...
function sessionPage(session, content, track = sessions) {
  ...
  const optionalBanner = session.isSideLab ? `<div class="side-lab-notice"><strong>Independent optional module, suggested near Session ${String(session.attachedToSession).padStart(2,"0")}:</strong> ${escapeHtml(session.sidebarDescription)} It supplies its own setup and is safe to start directly or skip.</div>` : session.isSupplemental ? `<div class="refresher-notice"><strong>Independent optional reference:</strong> This lesson supplies its own setup and is not a prerequisite for the TimeClock application path or another refresher.</div>` : "";
...
function labPage(session, content) { const label = learningItemLabel(session); return page({title:`${label} Lab — ${session.title}`,active:"labs",description:content.lab.objective,body:`<main id="main-content"><div class="container">
  ...
  ${renderLabBody(content.lab, content.lab.pasteReadyInstructions || (content.number >= 1 && content.number <= 24))}
...
function renderLabBody(lab, pasteReadyInstructions = false) { const keyPoint = lab.keyPoints ? `<aside class="card lab-key-point"><h3>${escapeHtml(lab.keyPointTitle || "Key point")}</h3><ul>${lab.keyPoints.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>${lab.keyPointCode ? `<pre><code>${escapeHtml(lab.keyPointCode)}</code></pre>` : ""}</aside>` : ""; const body = lab.labs ? lab.labs.map((item, index) => renderStructuredLab(item, index, lab.labs.length, lab.collapseInstructions, pasteReadyInstructions)).join("") : renderSingleStructuredLab(lab, pasteReadyInstructions); const attributes = [lab.clickHighlightInstructions ? "data-click-highlight-instructions" : "", lab.stepCheckboxes ? "data-step-checkboxes" : ""].filter(Boolean).join(" "); const steps = attributes ? `<div ${attributes}>${body}</div>` : body; return `${keyPoint}${steps}`; }
function renderSingleStructuredLab(lab, pasteReadyInstructions = false) { const steps = lab.steps.map((step, index) => renderLabStep(step, index, lab.stepCheckboxes, lab.steps.length, pasteReadyInstructions)).join(""); const cleanup = lab.preLabCleanup ? `<aside class="card pre-lab-cleanup"><h4>Pre-lab cleanup</h4><p>${escapeHtml(pasteReadyInstructions ? pasteReadyComment(lab.preLabCleanup.text) : lab.preLabCleanup.text)}</p>${lab.preLabCleanup.code ? `<pre><code>${escapeHtml(lab.preLabCleanup.code)}</code></pre>` : ""}</aside>` : ""; const instructions = lab.collapseInstructions ? `<details class="lab-instructions-reveal"><summary><strong>Instructions</strong><span class="quiz-expand-hint">Open instructions</span></summary>${steps}</details>` : `<h4>Instructions</h4>${steps}`; return `<article class="card lab-unit"><h3>Lab 1 of 1 — ${escapeHtml(lab.title || "Complete the Session Lab")}</h3><h4>Objective</h4><p class="lab-objective">${escapeHtml(lab.objective)}</p>${lab.practice ? `<h4>You’ll Practice</h4><ul>${lab.practice.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}${lab.task ? `<h4>Task</h4><p>${escapeHtml(lab.task)}</p>` : ""}${cleanup}${instructions}${lab.solution ? renderLabReveal({ title: "Solution", text: "Reveal only after completing the lab or when you are genuinely blocked.", code: lab.solution }) : ""}</article>`; }
function renderStructuredLab(lab, index, total, collapseInstructions = false, pasteReadyInstructions = false) { const stepTotal = lab.instructions.length; const instructions = `<ol class="lab-instructions">${lab.instructions.map((instruction, stepIndex) => { const detail = typeof instruction === "string" ? { text: instruction } : instruction; const instructionText = pasteReadyInstructions ? pasteReadyComment(detail.text) : detail.text; const purpose = detail.purposeAfter ? `<aside class="alert alert-info purpose-checkpoint"><strong>Purpose checkpoint:</strong> ${escapeHtml(detail.purposeAfter)}</aside>` : ""; const quickCheck = detail.checkAfter ? `<aside class="alert alert-info comprehension-check"><strong>Quick check:</strong> ${escapeHtml(detail.checkAfter.question)}<details><summary>Check the mental model</summary><p>${escapeHtml(detail.checkAfter.answer)}</p></details></aside>` : ""; return `<li><strong class="lab-step-label">Step ${stepIndex + 1}/${stepTotal}</strong><div class="lab-instruction"><p>${escapeHtml(instructionText)}</p>${pasteReadyInstructions || detail.copyInstruction ? `<button class="copy-instruction-button" type="button" data-copy-instruction>Copy instructions</button>` : ""}</div>${detail.code ? `<pre><code>${escapeHtml(detail.code)}</code></pre>` : ""}${purpose}${quickCheck}</li>`; }).join("")}</ol>`; const renderedInstructions = collapseInstructions ? `<details class="lab-instructions-reveal"><summary><strong>Instructions</strong><span class="quiz-expand-hint">Open instructions</span></summary>${instructions}</details>` : `<h4>Instructions</h4>${instructions}`; return `<article class="card lab-unit"><h3>Lab ${index + 1} of ${total} — ${escapeHtml(lab.title)}</h3><h4>Objective</h4><p class="lab-objective">${escapeHtml(lab.objective)}</p>${lab.practice ? `<h4>You’ll Practice</h4><ul>${lab.practice.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul>` : ""}${lab.task ? `<h4>Task</h4><p>${escapeHtml(lab.task)}</p>` : ""}${lab.comment ? `<p><strong>Starter comment:</strong></p><pre><code>${escapeHtml(lab.comment)}</code></pre>` : ""}${renderedInstructions}${lab.solution ? renderLabReveal({ title: "Solution", text: "Reveal only after completing the lab or when you are genuinely blocked.", code: lab.solution }) : ""}</article>`; }
function renderLabStep(step, index, stepCheckboxes = false, total = 0, pasteReadyInstructions = false) { const detail = typeof step === "string" ? { text: step } : step; const instructionText = pasteReadyInstructions ? pasteReadyComment(detail.text) : detail.text; const checkbox = stepCheckboxes ? `<label class="step-checkbox" title="Mark step ${index + 1} complete"><input type="checkbox" data-step-checkbox="${index}" aria-label="Mark step ${index + 1} complete"><span aria-hidden="true"></span></label>` : ""; const purpose = detail.purposeAfter ? `<aside class="alert alert-info purpose-checkpoint"><strong>Purpose checkpoint:</strong> ${escapeHtml(detail.purposeAfter)}</aside>` : ""; const completion = detail.completeWhen ? `<p class="step-complete-when"><strong>This step is complete when:</strong> ${escapeHtml(detail.completeWhen)}</p>` : ""; const questions = detail.questionsAfter?.length ? `<details class="card learner-questions"><summary><strong>What you should be asking right now</strong><span class="quiz-expand-hint">Open questions</span></summary>${detail.questionsAfter.map((item) => `<h5>${escapeHtml(item.question)}</h5><p>${escapeHtml(item.answer)}</p>`).join("")}</details>` : ""; return `<div class="step">${checkbox}<div class="step-num">Step ${index+1}/${total}</div><div class="step-body"><div class="lab-instruction"><p>${escapeHtml(instructionText)}</p>${pasteReadyInstructions || detail.copyInstruction ? `<button class="copy-instruction-button" type="button" data-copy-instruction>Copy instructions</button>` : ""}</div>${detail.code ? `<pre><code>${escapeHtml(detail.code)}</code></pre>` : ""}${completion}${questions}${(detail.reveals || []).map(renderLabReveal).join("")}</div></div>${purpose}`; }
...
function mentalModelDiagram(kind) {
  ...
  const distinctMentalKinds = {
    ...
    "24": ["branch", "invariant", "comparison", "tests"],
    "25": ["exceptionpropagation", "processtermination", "guaranteedcleanup", "resourcelease"],
    "26": ["pipeline", "call", "return", "feedback"],
    "27": ["pipeline", "call", "tests", "branch"],
    "28": ["call", "pipeline", "invariant", "feedback"],
    "28B": ["constructor", "depgraph", "path", "comparison"],
    "29": ["tests", "pipeline", "comparison", "feedback"],
    "30": ["tests", "contract", "comparison", "instances"],
    "30B": ["constructor", "instances", "boundary", "flowstate"],
    "31": ["invariant", "blueprint", "instances", "call"],
    "32": ["pipeline", "call", "return", "boundary"],
    "33": ["invariant", "call", "boundary", "path"],
    "34a": ["depgraph", "constructor", "instances", "tests"],
    "34b": ["boundary", "branch", "exceptionpropagation", "feedback"],
    ...
    "side-24b": ["pipeline", "branch", "comparison", "tests"],
  ...
  const scenes = {
    ...
    "side-01b": [`navigate · control · recover`, `<path d="M8 24h164v72H8zM18 34h38v52H18zM66 34h96v52H66z"/><circle cx="37" cy="48" r="8"${hot(1)}/><path d="M31 48h12M37 42v12"/><path d="M75 43h36M75 55h27"${hot(2)}/><path d="M120 40h32v25h-32zM127 48h18"${hot(3)}/><path d="M125 74h27M147 69l7 5-7 5"${hot(4)}/>`],
    "side-03b": [`value · format · text`, `<path d="M9 34h42v48H9zM17 47h26M17 61h18"${hot(1)}/>${line("M52 58h22")}<path d="M75 25h40v66H75zM84 38h22M84 53h22M84 68h22"${hot(2)}/>${line("M116 58h19")}<path d="M136 34h36v48h-36zM143 47h22M143 61h22"${hot(3)}/><path d="M143 74h22"${hot(4)}/>`],
    "side-04c": [`question · case · value`, `<circle cx="25" cy="60" r="15"${hot(1)}/><path d="M40 60h25M65 60l25-28M65 60l25 28"/><path d="M91 20h35v25H91zM91 76h35v25H91z"${hot(2)}/><path d="M127 32h20M127 88h20"${hot(3)}/><path d="M148 21h25v80h-25zM154 37h13M154 53h13M154 69h13"${hot(4)}/>`],
    "side-05b": [`field · access · property`, `<path d="M12 25h64v70H12zM23 38h42v44H23z"${hot(1)}/><path d="M77 48h27v24H77zM85 55h11"${hot(2)}/>${line("M105 60h23")}<path d="M129 25h42v70h-42zM138 40h24M138 55h24"${hot(3)}/><path d="M142 76h16M150 68v16"${hot(4)}/>`],
    "side-19b": [`public · internal · protected`, `<path d="M10 17h160v88H10zM20 28h42v66H20zM69 28h42v66H69zM118 28h42v66h-42z"/><path d="M29 43h24M29 59h24"${hot(1)}/><path d="M78 43h24M78 59h24"${hot(2)}/><path d="M127 43h24M127 59h24"${hot(3)}/><path d="M128 79h22M139 68v22"${hot(4)}/>`],
    "side-24b": [`outcome · translate · contract`, `<path d="M8 24h43v72H8zM16 37h27M16 52h27"${hot(1)}/>${line("M52 60h22")}<path d="M75 30l22-12 22 12v60L97 102 75 90z"${hot(2)}/>${line("M120 60h18")}<path d="M139 24h34v72h-34zM146 38h20M146 53h20"${hot(3)}/><path d="M147 77l7 7 12-17"${hot(4)}/>`],
    "ref-01": [`foundation · layers · version`, `<path d="M12 91h156M20 76h140M30 61h120M42 46h96M57 31h66"${hot(1)}/><path d="M30 61h120"${hot(2)}/><path d="M84 18v76M78 27h12M78 47h12M78 67h12"${hot(3)}/><circle cx="123" cy="31" r="10"${hot(4)}/>`],
    "ref-02": [`type · behavior · pipeline`, `<path d="M8 25h42v70H8zM17 38h24M17 54h24"${hot(1)}/>${line("M51 60h20")}<path d="M72 22h38v76H72zM80 36h22M80 52h22"${hot(2)}/>${line("M111 60h20")}<path d="M132 25h40v70h-40zM140 38h24M140 54h24"${hot(3)}/><path d="M141 76h22"${hot(4)}/>`],
    "ref-03": [`start · wait · resume`, `<path d="M9 30h35v60H9zM16 43h21M16 57h21"${hot(1)}/>${line("M45 60h22")}<path d="M68 40h35v40H68zM76 50h19M76 63h19"${hot(2)}/><path d="M111 25h25v25h-25zM111 70h25v25h-25z"${hot(3)}/>${line("M137 38h25")}${line("M137 82h25")}<path d="M163 28v64"${hot(4)}/>`],
    "ref-04": [`shape · null · narrowed`, `<path d="M10 20h55v80H10zM20 32h35M20 47h35M20 62h35"${hot(1)}/>${line("M66 60h23")}<path d="M90 23h35v74H90zM98 37h19M98 54h19"${hot(2)}/><circle cx="148" cy="42" r="15"${hot(3)}/><path d="M136 78h25l-12 18z"${hot(4)}/>`],
    "ref-05": [`model · initialize · compose`, `<path d="M9 26h42v68H9zM17 39h26M17 54h26"${hot(1)}/><path d="M61 20h48v80H61zM70 34h30M70 50h30M70 66h30"${hot(2)}/><path d="M119 28h52v28h-52zM119 68h52v28h-52z"${hot(3)}/><path d="M130 42h30M130 82h30"${hot(4)}/>`]
  };
...
function diagram(kind) { const mental = mentalModelDiagram(kind); if (mental) return mental; const diagrams = {
  ...
  exceptionpropagation:`<svg viewBox="0 0 180 120" role="img" aria-labelledby="d40t d40d"><title id="d40t">Exception climbs the call stack</title><desc id="d40d">A bright exception flare rises unchanged through model, service, and application floors until a catcher handles it.</desc><path d="M20 103h118M31 78h118M42 53h118M53 28h118"/><path d="M77 103V90l-9-7 18-8-8-8 17-11-7-8 18-12"/><circle cx="106" cy="34" r="6"/><path d="M112 34h43M147 28l8 6-8 6"/><text x="42" y="98">model</text><text x="55" y="73">service</text><text x="68" y="48">app</text><text x="137" y="22">catch</text></svg>`,
  processtermination:`<svg viewBox="0 0 180 120" role="img" aria-labelledby="d41t d41d"><title id="d41t">Unhandled exception terminates the process</title><desc id="d41d">An exception spark reaches the top of a console process, trips its breaker, prints a stack trace, and leaves the process dark.</desc><rect x="13" y="19" width="102" height="82" rx="8"/><path d="M25 34h48M25 48h61M25 62h42M25 76h55"/><path d="M91 75l8-14 8 14 8-14"/><circle cx="143" cy="47" r="20"/><path d="M143 27v20l14 12M130 78h27M143 67v28"/><path d="M122 18l42 84"/><text x="53" y="94">stack trace</text></svg>`,
  guaranteedcleanup:`<svg viewBox="0 0 180 120" role="img" aria-labelledby="d42t d42d"><title id="d42t">Cleanup closes both exits</title><desc id="d42d">Success and exception paths leave a work area through different doors, but one cleanup attendant closes the same resource valve after either path.</desc><path d="M16 20h72v66H16zM27 32h49v42H27z"/><path d="M88 40h35M88 68h35"/><circle cx="135" cy="40" r="10"/><path d="M128 33l14 14M142 33l-14 14"/><path d="M125 68l7 7 13-17"/><path d="M151 29v62M142 91h18"/><circle cx="151" cy="96" r="8"/><path d="M151 88v-9"/><text x="52" y="99">resource</text><text x="135" y="19">either path</text></svg>`,
  resourcelease:`<svg viewBox="0 0 180 120" role="img" aria-labelledby="d43t d43d"><title id="d43t">Dispose returns an external resource lease</title><desc id="d43d">A database connection key is checked out to code and then returned to its owner through the IDisposable contract.</desc><path d="M12 27h49v66H12zM22 40h29M22 53h29M22 66h29"/><circle cx="92" cy="48" r="16"/><path d="M108 48h26l7 7 8-8 8 8 11-11"/><path d="M86 63v25h57M137 82l6 6-6 6"/><path d="M133 22h35v22h-35z"/><text x="36" y="82">database</text><text x="92" y="52">lease</text><text x="150" y="37">code</text><text x="116" y="103">Dispose</text></svg>`
