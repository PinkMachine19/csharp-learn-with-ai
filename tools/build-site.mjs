...
function renderLabBody(lab) { const body = lab.labs ? lab.labs.map((item, index) => renderStructuredLab(item, index, lab.labs.length, lab.collapseInstructions)).join("") : lab.steps.map(renderLabStep).join(""); return lab.clickHighlightInstructions ? `<div data-click-highlight-instructions>${body}</div>` : body; }
