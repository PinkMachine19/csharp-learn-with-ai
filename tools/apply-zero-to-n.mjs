...
Object.assign(contentOverrides, {
  "26": {
    concept: [
      { title: "Task represents work", paragraphs: ["Task represents completion and Task<T> represents completion plus a value.", "The ScratchPad example uses Task.FromResult because no genuine I/O exists yet."], code: "Task<string> nameTask = Task.FromResult(\"Parm\");" },
      { title: "await observes the result", paragraphs: ["await reads the eventual string and observes failure.", "await does not automatically mean another thread."], code: "string name = await nameTask;" }
    ]
  },
  "27": {
    concept: [
  ...
  "28": {
    concept: [
      { title: "Calling an async method returns a Task", paragraphs: ["The Task exists before it is awaited.", "The ScratchPad deliberately creates a faulted Task without claiming production already has async I/O."], code: "Task failureTask = FailAfterDelayAsync();" },
      { title: "await observes asynchronous failure", paragraphs: ["The exception is rethrown at the await point and can be caught there.", "The final experiment awaits every deliberate Task and finishes without warnings."], code: "await failureTask;" }
    ]
  },
  "29": {
    concept: [
      ...
      { title: "Act performs one real behavior", paragraphs: ["The actual PayrollService method is synchronous, so the test calls CalculateTotalHours directly.", "Test shape follows the real contract rather than invented async history."], code: "double total = service.CalculateTotalHours(employeeId, from, to);" },
      { title: "Assert checks an observable outcome", paragraphs: ["Assert the returned total rather than private implementation details.", "A failing assertion should explain which behavior changed."], code: "Assert.Equal(7, total);" }
    ]
  },
  "30": {
    concept: [
...
Object.assign(plans, {
  ...
  "25": { objective: "Prove exception propagation and guaranteed cleanup in a focused ScratchPad experiment.", starting: "Session 24 added tested production exceptions. The production total-hours query filters out open entries before GetDuartion, and no TimeClock resource requires a custom IDisposable implementation.", steps: ["Create Session25/ResourceExperiments.cs in ScratchPad.", "Add a private nested IDisposable probe that records and prints when Dispose runs.", "Declare the probe before try, use it inside a using block, and deliberately throw InvalidOperationException inside using.", "Catch the exception outside using and inspect WasDisposed afterward.", "Call Run without removing earlier ScratchPad calls, then run ScratchPad, build, and test."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/Session25/ResourceExperiments.cs", "Add", "Demonstrate propagation and guaranteed cleanup without inventing a production failure."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Run the permanent Session 25 experiment.")], behavior: "ScratchPad prints cleanup before the caught exception and proves WasDisposed is true; production App remains unchanged." },
  "26": { objective: "Introduce Task, Task<T>, async, and await through an honest ScratchPad experiment.", starting: "Production repository and service methods remain synchronous because the current List-based repository performs no external I/O.", steps: ["Create Session26/AsyncFlowExperiments.cs in ScratchPad.", "Return an already-known employee name through Task.FromResult.", "Observe Task<string> before awaiting it, then await and print the string result.", "Await RunAsync from ScratchPad's top-level Program.cs.", "Run ScratchPad, build, and test."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/Session26/AsyncFlowExperiments.cs", "Add", "Preserve the isolated Task and await experiment."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Await RunAsync without removing earlier experiments.")], behavior: "ScratchPad observes an already-completed Task<string> before await and the string Parm after await; production remains synchronous." },
  "27": { objective: "Compare two independent asynchronous operations sequentially and concurrently in ScratchPad.", starting: "Session 26 introduced Task, Task<T>, async, and await in AsyncFlowExperiments. Production remains synchronous.", steps: ["Create Session27/SequentialConcurrentExperiments.cs.", "Use Task.Delay to simulate visible waiting in a Task<string> helper.", "Await two calls immediately for sequential flow.", "Create two named Tasks before Task.WhenAll for concurrent flow.", "Await the experiment from ScratchPad Program.cs, then run, build, and test."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/Session27/SequentialConcurrentExperiments.cs", "Add", "Preserve the sequential-versus-concurrent observation."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Await Session 27 after earlier experiments.")], behavior: "Sequential output completes employee 1 before starting employee 2; concurrent output starts both before either finishes." },
  "28": { objective: "Observe asynchronous completion and failure through an awaited Task in ScratchPad.", starting: "Session 27 compared sequential and concurrent waits; production remains synchronous.", steps: ["Create Session28/AsyncFailureExperiments.cs.", "Return a Task that faults after a short simulated delay.", "Store and await that Task inside try/catch.", "Await RunAsync from ScratchPad Program.cs.", "Run, build, and test with no discarded Task or warning."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/Session28/AsyncFailureExperiments.cs", "Add", "Preserve the fault-observation experiment."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Await Session 28.")], behavior: "ScratchPad observes the intentional exception through await and continues after catch." },
  "28B": { objective: "Request, propagate, and observe cooperative cancellation in ScratchPad.", starting: "Session 28 observed a faulted Task; production remains synchronous.", steps: ["Create Session28B/CooperativeCancellationExperiments.cs.", "Forward one CancellationToken through a helper to Task.Delay.", "Request cancellation from a disposable CancellationTokenSource.", "Await the stored Task and handle OperationCanceledException.", "Await the experiment from Program.cs, then run, build, and test."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/Session28B/CooperativeCancellationExperiments.cs", "Add", "Preserve the cooperative-cancellation experiment."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Await Session 28B after Session 28.")], behavior: "ScratchPad proves that cancellation was requested, observed, and handled." },
  "29": { objective: "Test the real synchronous PayrollService.CalculateTotalHours behavior with Arrange–Act–Assert.", starting: "ClockEntry validation tests exist, but PayrollService has no learner tests.", steps: ["Create PayrollServiceTests.cs.", "Arrange deterministic completed entries in a fresh ClockEntryRepository.", "Act by calling synchronous CalculateTotalHours.", "Assert seven hours and the zero-entry boundary.", "Run focused tests, build, and run the full suite."], expected: [f("tests/PinkMachine19.TimeClock.Domain.Tests/PayrollServiceTests.cs", "Add", "Verify actual synchronous total-hours behavior.")], behavior: "Permanent tests cover completed totals and the empty boundary without invented async APIs or mock packages." },
  "30": { objective: "Compare a hand-written recording fake with Moq while isolating ClockEntryService.", starting: "Session 29 established permanent AAA tests; ClockEntryService already depends on IClockEntryRepository.", steps: ["Create ClockEntryServiceTests and a minimal recording repository fake.", "Test the successful TryCLockIn collaboration through recorded state.", "Add Moq to the test project.", "Test the same behavior with Setup and Verify.", "Run focused tests, build, and run the full suite."], expected: [f("tests/PinkMachine19.TimeClock.Domain.Tests/PinkMachine19.TimeClock.Domain.Tests.csproj", "Modify", "Add Moq for interaction verification."), f("tests/PinkMachine19.TimeClock.Domain.Tests/ClockEntryServiceTests.cs", "Add", "Compare hand-written and library test doubles.")], behavior: "Two isolated tests prove the same service requirement through recorded state and Moq interaction verification.", preserveStructuredLabs: true },
  "30B": { objective: "Observe transient, scoped, and singleton reuse through explicit DI scopes.", starting: "Session 30 used constructor injection for test isolation without requiring a container.", steps: ["Add Microsoft.Extensions.DependencyInjection to ScratchPad.", "Create Session30B/LifetimeExperiments.cs and register three marker types.", "Build a provider and two scopes.", "Resolve named instances and compare identity for each lifetime.", "Run ScratchPad, build, and test."], expected: [f("src/PinkMachine19.TimeClock.ScratchPad/PinkMachine19.TimeClock.ScratchPad.csproj", "Modify", "Add the DI container package."), f("src/PinkMachine19.TimeClock.ScratchPad/Session30B/LifetimeExperiments.cs", "Add", "Preserve the lifetime experiment."), f("src/PinkMachine19.TimeClock.ScratchPad/Program.cs", "Modify", "Run Session 30B after earlier experiments.")], behavior: "ScratchPad prints False, True, False, True for transient, scoped-within, scoped-across, and singleton identity." },
  ...
  "34A": { objective: "Centralize and verify the existing scoped persistence registrations.", starting: "Session 33 has a working registration graph in Web Program.cs.", steps: ["Create Infrastructure ServiceRegistration and AddTimeClockPersistence.", "Move the detailed DbContext, repository, and service registrations behind the extension method.", "Keep Program.cs as the composition root choosing the connection string.", "Create a real provider and scope in a composition test.", "Resolve AsyncClockEntryService and state that resolution proves construction rather than behavior."], expected: [f("src/PinkMachine19.TimeClock.Infrastructure/ServiceRegistration.cs", "Add", "Group the persistence registration recipe."), f("src/PinkMachine19.TimeClock.Web/Program.cs", "Modify", "Invoke the recipe from the composition root."), f("tests/PinkMachine19.TimeClock.Domain.Tests/ServiceRegistrationTests.cs", "Add", "Verify constructor resolution.")], behavior: "The scoped constructor graph resolves through one organized registration recipe." },
  "34B": { objective: "Add validation, structured logging, and safe API error responses around the existing endpoint.", starting: "Session 34A verified the production dependency graph.", steps: ["Validate transport input at the controller boundary.", "Map invalid input and conflict to stable ProblemDetails.", "Add structured logs with safe event context.", "Log unexpected exceptions without exposing internal details to callers.", "Run HTTP smoke checks and the complete suite."], expected: [f("src/PinkMachine19.TimeClock.Web/Controllers/ClockEntriesController.cs", "Modify", "Add boundary validation, logging, and error mapping.")], behavior: "The endpoint returns safe, deliberate error contracts and emits useful diagnostic logs." },
  "35": { objective: "Complete and verify the end-to-end record-completed-entry workflow built across the curriculum.", starting: "Sessions 01–34B cumulatively created the solution, domain, services, tests, persistence, async flow, cancellation, and HTTP boundary. No prebuilt production artifact is assumed.", steps: ["Trace one clock-in and clock-out request across Web, service, repository, EF Core, and Domain.", "Fill only genuine integration gaps discovered by the trace.", "Add an end-to-end test that records, completes, persists, queries, and summarizes one entry.", "Run App and Web smoke checks plus the complete test suite.", "Review the build sequence and identify the session that created every file touched by the workflow."], expected: [f("tests/PinkMachine19.TimeClock.Domain.Tests/ClockEntryWorkflowTests.cs", "Add", "Verify the complete workflow."), f("src/PinkMachine19.TimeClock.Infrastructure/ClockEntryWorkflow.cs", "Add", "Coordinate persistence only if the existing service boundary requires it.")], behavior: "The application built from Session 01 works end to end, and the final test proves the same artifacts the student created throughout the course."
  }
});
