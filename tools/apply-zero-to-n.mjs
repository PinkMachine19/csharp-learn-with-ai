...
const plans = {
  "1": {
    ...
    steps: [
      { text: "Create the classic solution file in the learner workspace.", code: "dotnet new sln -n PinkMachine19.TimeClock -f sln" },
      { text: "Create the four production projects, the permanent ScratchPad console project, and the Domain test project. ScratchPad is a notebook that stays in the solution for the entire course.", code: "dotnet new classlib -o src/PinkMachine19.TimeClock.Domain\ndotnet new console -o src/PinkMachine19.TimeClock.App\ndotnet new classlib -o src/PinkMachine19.TimeClock.Infrastructure\ndotnet new web -o src/PinkMachine19.TimeClock.Web\ndotnet new console -o src/PinkMachine19.TimeClock.ScratchPad\ndotnet new xunit -o tests/PinkMachine19.TimeClock.Domain.Tests" },
      { text: "Add all six projects to the solution. Creating a project does not add it automatically.", code: "dotnet sln PinkMachine19.TimeClock.sln add src/PinkMachine19.TimeClock.Domain/PinkMachine19.TimeClock.Domain.csproj\ndotnet sln PinkMachine19.TimeClock.sln add src/PinkMachine19.TimeClock.App/PinkMachine19.TimeClock.App.csproj\ndotnet sln PinkMachine19.TimeClock.sln add src/PinkMachine19.TimeClock.Infrastructure/PinkMachine19.TimeClock.Infrastructure.csproj\ndotnet sln PinkMachine19.TimeClock.sln add src/PinkMachine19.TimeClock.Web/PinkMachine19.TimeClock.Web.csproj\ndotnet sln PinkMachine19.TimeClock.sln add src/PinkMachine19.TimeClock.ScratchPad/PinkMachine19.TimeClock.ScratchPad.csproj\ndotnet sln PinkMachine19.TimeClock.sln add tests/PinkMachine19.TimeClock.Domain.Tests/PinkMachine19.TimeClock.Domain.Tests.csproj" },
      { text: "List and build the solution. Expect six project paths and zero build errors.", code: "dotnet sln PinkMachine19.TimeClock.sln list\ndotnet build PinkMachine19.TimeClock.sln" },
      { text: "Create the purpose-named production folders. Do not add speculative C# implementations yet.", code: "mkdir -p src/PinkMachine19.TimeClock.Domain/{Constants,Exceptions,Extensions,Helpers,Interfaces,Models,Services}\nmkdir -p src/PinkMachine19.TimeClock.Infrastructure/Repositories\nmkdir -p src/PinkMachine19.TimeClock.Web/{Configuration,Dtos,Mappings,Options,Requests,Responses,Validators}" },
      { text: "Add an empty .gitkeep to every otherwise-empty folder so the structure survives a clone.", code: "for d in Constants Exceptions Extensions Helpers Interfaces Models Services; do touch \"src/PinkMachine19.TimeClock.Domain/$d/.gitkeep\"; done\ntouch src/PinkMachine19.TimeClock.Infrastructure/Repositories/.gitkeep\nfor d in Configuration Dtos Mappings Options Requests Responses Validators; do touch \"src/PinkMachine19.TimeClock.Web/$d/.gitkeep\"; done" }
    ],
  ...
  "3": {
    ...
    steps: [
      "Open src/PinkMachine19.TimeClock.ScratchPad/Program.cs and keep the file as the permanent language notebook.",
      { text: "Add a clearly labeled Session 03 section and begin with the supplied measurement value.", code: "// Session 03 — Variables, expressions, and numeric types\ndouble measuredHours = 8.0;" },
      { text: "Predict scheduledHours, converted recordedHours, and recorded-minus-scheduled variance before writing the expressions.", reveals: [{ title: "Expected reasoning", items: ["scheduledHours is decimal 7.5.", "recordedHours is decimal 8.0 after conversion.", "varianceHours is decimal 0.5."] }] },
      { text: "Write the decimal declaration, explicit conversion, subtraction, and formatted output. Try first; reveal source only if blocked.", reveals: [{ title: "Expected source code", blurred: true, code: "decimal scheduledHours = 7.5m;\ndecimal recordedHours = (decimal)measuredHours;\ndecimal varianceHours = recordedHours - scheduledHours;\n\nConsole.WriteLine($\"Scheduled: {scheduledHours:F2}\");\nConsole.WriteLine($\"Recorded: {recordedHours:F2}\");\nConsole.WriteLine($\"Variance: {varianceHours:+0.00;-0.00;0.00}\");" }] },
      { text: "Run ScratchPad, then build and test the entire solution.", code: `${runScratch}\n${runAll}` }
    ],
  ...
  "4": {
    ...
    steps: ["Keep all earlier experiments.", "Add a Session 04 section that classifies a local variance value as over, under, or on schedule.", "Predict positive, zero, and negative outcomes before writing the switch expression.", "Run all three values and compare them with the prediction.", { text: "Run ScratchPad and build the solution.", code: `${runScratch}\ndotnet build PinkMachine19.TimeClock.sln --configuration Release` }],
    ...
    behavior: "ScratchPad retains prior work and demonstrates all three decision paths; production remains unchanged."
  },
  "5": {
    ...
    steps: ["Keep the observable examples and extract repeated calculation into a local method.", "Add parameters for recorded and scheduled hours and return the decimal difference.", "Extract classification into a second local method.", "Call both methods from the existing labeled sections and verify scope boundaries.", { text: "Run ScratchPad and the solution tests.", code: `${runScratch}\ndotnet test PinkMachine19.TimeClock.sln --configuration Release` }],
...
for (const [number, plan] of Object.entries(plans)) {
  ...
  session.lab.validation = build.environment === "scratchpad" ? `${runScratch} && dotnet build PinkMachine19.TimeClock.sln --configuration Release` : runAll;
